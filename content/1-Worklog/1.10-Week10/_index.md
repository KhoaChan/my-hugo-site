...
| --- | --- | --- | --- |
| Day 1 (23/06) | Backend Module Preparation | Prepared the Backend structure and organized modules for Authentication, Video Management, Upload Processing, and Status Tracking. | Completed the initial Backend module structure. |
| Day 2 (24/06) | Authentication Module | Worked on the authentication flow, including user sign-in/sign-up planning and Cognito integration with API Gateway. | Completed the basic authentication module design. |
| Day 3 (25/06) | Upload API Design | Designed the API for requesting a Presigned URL so that users or administrators can upload videos directly to Amazon S3. | Completed the upload API workflow. |
| Day 4 (26/06) | Metadata Handling | Planned how video metadata would be stored in DynamoDB after a video is uploaded. | Completed the metadata saving process. |
| Day 5 (27/06) | Video Processing Pipeline | Reviewed the asynchronous processing flow using S3 Event, SQS, Lambda, and MediaConvert. | Completed the processing pipeline workflow draft. |
| Day 6 (28/06) | Error Handling Review | Identified possible errors such as failed upload, duplicate events, failed MediaConvert jobs, and missing metadata. | Completed the initial error-handling notes. |
| Day 7 (29/06) | Documentation Update | Updated the worklog, added screenshots, reviewed technical explanations, and checked report consistency. | Completed Week 10 documentation and prepared for integration work. |
...
Week 10 helped me better understand the role of the design stage in the development process of a Cloud system. Although I did not directly build business functions yet, analyzing data, designing APIs, and defining the processing flow between AWS services helped me clearly visualize how the components would work together during implementation.
